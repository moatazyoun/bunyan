    ...
    if (user && selectedSite && isDbLoaded) {
      ...
      const timer = setTimeout(async () => {
        try {
          ...
          await setDoc(doc(db, 'siteData', selectedSite.id), cleanPayload, { merge: true });
        } catch (err) {
    ...
    if (user && selectedSite) {
      ...
      const loadSiteData = async () => {
        ...
        try {
          ...
          try {
            const docSnap = await getDoc(doc(db, 'siteData', selectedSite.id));
            if (docSnap.exists()) {
              hasDoc = true;
              data = docSnap.data();
            ...
            console.warn("Firestore query failed, using local fallback cache...", fireErr);
