...
async function emulatedFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
  ...
  const isAIBackendOnly = urlStr.includes('/api/gemini/analyze-report') || urlStr.includes('/api/gemini/analyze-voucher') || urlStr.includes('/api/gemini/analyze-boq') || urlStr.includes('/api/gemini/analyze-sarki') || urlStr.includes('/api/gemini/generate-schedule') || urlStr.includes('/api/gemini/analyze-supplies');
